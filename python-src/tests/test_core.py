...
'''
    ...
    def test_default_paths_expand_home_directory(self):
        with patch.dict(os.environ, {}, clear=True), patch.object(Path, 'mkdir'):
            config_manager = ConfigManager()

        self.assertEqual(config_manager.config_dir, Path.home() / '.config' / 'llm-coding')
        self.assertEqual(config_manager.state_dir, Path.home() / '.local' / 'state' / 'llm-coding')
    ...
    def test_api_errors_include_runpod_response_body(self):
        ...
        with patch('llm_coding.core.requests.request', return_value=response):
            with self.assertRaisesRegex(
                ...
                RunPodClient('test-key')._make_request('POST', '/pods/test/start')
...
class TestRuntimeStartPod(unittest.TestCase):
    ...
    def test_capacity_failure_confirms_network_volume_is_safe_to_reuse(self):
        client = MagicMock()
        client.start_pod.side_effect = RuntimeError('not enough free GPUs on the host machine')

        with self.assertRaisesRegex(RuntimeError, r'detachable network volume'):
            runtime_module._start_pod_or_raise(client, 'test', {'RUNPOD_NETWORK_VOLUME_ID': 'vol-1'})
...
class TestSystemdInstall(unittest.TestCase):
    ...
    def test_proxy_start_timeout_matches_configured_activation_budget(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            unit = config_home / 'systemd' / 'user' / 'llm-coding-proxy.service'
    ...
    def test_ensure_socket_reinstalls_stale_unit(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            (unit_dir / 'llm-coding-proxy.service').write_text('TimeoutStartSec=35min\n')
...
class TestRuntimeReadySummary(unittest.TestCase):
    ...
    def test_reports_verified_runtime_and_integrations(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir, \
             ...
             patch.object(cli_module, 'get_systemd_service_info', return_value=('active', 'id')), \
             patch.object(cli_module.RunPodClient, 'find_pod_by_name', return_value=pod), \
             patch.object(cli_module, 'create_opencode_config', return_value=Path(temp_dir) / 'opencode.json'), \
             patch('builtins.print') as print_mock:
            cli_module.print_runtime_ready_summary(config, Path(temp_dir), {'data': [{'id': 'test-model'}]})
...
class TestActivationFailureDisplay(unittest.TestCase):
    ...
    def test_prints_recorded_actionable_error_without_journal(self):
        with tempfile.TemporaryDirectory() as temp_dir, \
            ...
            Path(temp_dir, 'runtime.activation-error').write_text('No GPU capacity. Retry later.')
    ...
    def test_reads_latest_activation_status(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            Path(temp_dir, 'runtime.activation-status').write_text('Waiting for RunPod SSH\n')
            self.assertEqual(
...
class TestLlmUp(unittest.TestCase):
    ...
    def test_recovers_active_proxy_when_tunnel_is_unavailable(self):
        with tempfile.TemporaryDirectory() as temp_dir, \
             ...
             patch.object(cli_module, 'ensure_socket'), \
             patch.object(cli_module, 'ConfigManager') as config_manager_class, \
             patch.object(cli_module, 'get_systemd_service_info', side_effect=[('active', 'old'), ('active', 'old')]), \
             patch.object(cli_module.requests, 'get', side_effect=cli_module.requests.ConnectionError), \
    ...
    def test_llm_down_stops_proxy_before_stopping_runtime(self):
        with patch.object(cli_module, '_shutdown_summary_items', return_value=(
                 ['socket listener', 'local proxy'], ['runtime.env', 'opencode.json']
             )), \
             patch.object(cli_module, 'run_command') as run_command, \
...
class TestRuntimeDown(unittest.TestCase):
    ...
    def test_removes_generated_opencode_config_and_only_its_acp_entry(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            acp_file.write_text(json.dumps({'agent_servers': {
            ...
            manager.load_config.return_value = {
                'RUNPOD_API_KEY': 'test-key', 'RUNPOD_POD_NAME': 'test-pod',
                'JETBRAINS_AGENT_NAME': 'OpenCode RunPod',
            }
            systemctl = MagicMock()
            systemctl.return_value.returncode = 0
            with patch.object(runtime_module, 'ConfigManager', return_value=manager), \
                 patch.object(runtime_module, '_systemctl', systemctl), \
                 patch.object(runtime_module.Path, 'home', return_value=home), \
                 patch.object(runtime_module.RunPodClient, 'find_pod_by_name', return_value=None):
                runtime_module.down()
    ...
    def test_reports_pod_failure_after_local_cleanup(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch.object(runtime_module, 'ConfigManager', return_value=manager), \
                 patch.object(runtime_module, '_systemctl', systemctl), \
                 patch.object(runtime_module.Path, 'home', return_value=root / 'home'), \
                 patch.object(runtime_module.RunPodClient, 'find_pod_by_name', side_effect=RuntimeError('API unavailable')):
                with self.assertRaisesRegex(RuntimeError, r'Shutdown incomplete: could not stop RunPod pod test-pod'):
                    runtime_module.down()
