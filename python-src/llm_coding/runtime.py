...
def _command_error(result):
    """Return a short diagnostic for an unsuccessful subprocess result."""
...
def _render_systemd_units(config):
    ...
    proxy = shutil.which("systemd-socket-proxyd") or "/usr/lib/systemd/systemd-socket-proxyd"
    if not Path(proxy).exists():
        raise RuntimeError("systemd-socket-proxyd is required but was not found")
...
def install():
    ...
    for destination, template in ((manager.config_file, "config/config.env.example"),
    ...
    if not key.exists():
        ...
        _run(["ssh-keygen", "-q", "-t", "ed25519", "-f", str(key), "-N", "", "-C", "runpod-llm-coding"])
    opencode = Path.home() / ".opencode/bin/opencode"
    if not opencode.exists() or _run([str(opencode), "--version"], check=False, capture_output=True).stdout.strip() != config.get("OPENCODE_VERSION"):
        installer = requests.get("https://opencode.ai/install", timeout=60).text
        _run(["bash", "-s", "--", "--version", config["OPENCODE_VERSION"], "--no-modify-path"], input=installer)
    acp_file = Path.home() / ".jetbrains/acp.json"
    acp_file.parent.mkdir(parents=True, exist_ok=True)
    acp = json.loads(acp_file.read_text()) if acp_file.exists() else {"default_mcp_settings": {}, "agent_servers": {}}
    defaults = acp.setdefault("default_mcp_settings", {})
    defaults["use_idea_mcp"] = config.get("ENABLE_IDEA_MCP", "true").lower() == "true"
    defaults["use_custom_mcp"] = config.get("ENABLE_CUSTOM_MCP", "false").lower() == "true"
    acp.setdefault("agent_servers", {})[config.get("JETBRAINS_AGENT_NAME", "OpenCode RunPod")] = {
        "command": str(Path(sys.argv[0]).resolve().parent / "opencode-runpod"), "args": ["acp"]}
...
def ensure_socket(config):
    ...
    if any((user_units / name).read_text() != contents if (user_units / name).exists() else True
...
def _ssh_base(config, host, port):
    ...
    _run(["ssh-keygen", "-R", f"[{host}]:{port}", "-f", str(known_hosts)], check=False,
         capture_output=True)
    return ["ssh", "-T", "-i", _value(config, "RUNPOD_SSH_KEY"), "-p", str(port),
            "-o", "BatchMode=yes", "-o", "ConnectTimeout=5", "-o", "ServerAliveInterval=30",
            "-o", "ServerAliveCountMax=3", "-o", "StrictHostKeyChecking=accept-new",
            "-o", f"UserKnownHostsFile={known_hosts}", f"root@{host}"]
    ...
    try:
        ...
        if "not enough free gpus on the host machine" in message.lower():
            storage_warning = (
                ...
                if config.get("RUNPOD_NETWORK_VOLUME_ID") else
                "This pod uses pod-local storage (RUNPOD_NETWORK_VOLUME_ID is empty); "
...
def up():
    ...
    with open(manager.state_dir / "runtime.lock", "w") as lock:
        ...
        if pod is None:
            ...
            body = {"name": config["RUNPOD_POD_NAME"], "imageName": config["RUNPOD_IMAGE"],
                    "cloudType": config.get("RUNPOD_CLOUD_TYPE", "SECURE"), "computeType": "GPU",
                    "gpuTypeIds": [config["RUNPOD_GPU_TYPE"]], "gpuTypePriority": "availability",
                    "gpuCount": 1, "interruptible": False, "supportPublicIp": True,
                    ...
                    "minVCPUPerGPU": int(config.get("RUNPOD_MIN_VCPU_PER_GPU", 8)), "ports": ["22/tcp"],
                    "env": {"SSH_PUBLIC_KEY": key.with_suffix(key.suffix + ".pub").read_text().strip()}}
            volume = config.get("RUNPOD_NETWORK_VOLUME_ID")
            body["networkVolumeId" if volume else "volumeInGb"] = volume or int(config.get("RUNPOD_VOLUME_GB", 100))
            pod_id = client.create_pod(body)
        else:
            pod_id = pod["id"]
            status = pod.get("desiredStatus")
            if status == "TERMINATED":
                raise RuntimeError(f"RunPod {pod_id} is TERMINATED; delete it or change RUNPOD_POD_NAME")
            if status == "EXITED":
                _set_activation_status(manager, "Starting the stopped RunPod pod")
                _start_pod_or_raise(client, pod_id, config)
        deadline = time.monotonic() + int(config.get("RUNPOD_START_TIMEOUT_SECONDS", 1200))
        host = port = None
        _set_activation_status(manager, "Waiting for RunPod to expose SSH")
        while time.monotonic() < deadline:
            pod = client.get_pod(pod_id)
            host, port = pod.get("publicIp"), pod.get("portMappings", {}).get("22")
            if host and port: break
            time.sleep(5)
        if not host or not port: raise RuntimeError("RunPod did not expose SSH before timeout")
        ssh = _ssh_base(config, host, port)
        _set_activation_status(manager, "Waiting for the RunPod SSH service")
        while time.monotonic() < deadline:
            if _run([*ssh, "true"], check=False, capture_output=True).returncode == 0: break
            ...
            if refreshed_host and refreshed_port and (refreshed_host, refreshed_port) != (host, port):
                host, port = refreshed_host, refreshed_port
                ssh = _ssh_base(config, host, port)
            time.sleep(5)
        else: raise RuntimeError("SSH did not become available before timeout")
        script = _asset("remote/ensure-vllm.sh")
        _set_activation_status(manager, "Preparing vLLM on RunPod (first start can take several minutes)")
        _run([*ssh, "bash", "-s", "--", config["VLLM_VERSION"], config["VLLM_CUDA_VERSION"], config["MODEL_ID"],
              config.get("MODEL_REVISION", ""), config["SERVED_MODEL_NAME"], config.get("CONTEXT_SIZE", "65536"),
              config.get("VLLM_GPU_MEMORY_UTILIZATION", "0.92"), config.get("VLLM_TOOL_CALL_PARSER", "qwen3_xml"),
              config.get("REMOTE_VLLM_PORT", "8000"), config.get("VLLM_START_TIMEOUT_SECONDS", "1800")], input=script.read_text())
        (manager.state_dir / "runtime.env").write_text(f"SSH_HOST={host}\nSSH_PORT={port}\nSSH_KEY={key}\n")
        ...
        _set_activation_status(manager, "Verifying vLLM through the SSH tunnel")
        while time.monotonic() < healthy_until:
            try:
                models = requests.get(endpoint, timeout=2).json().get("data", [])
                if any(model.get("id") == config["SERVED_MODEL_NAME"] for model in models):
...
def _remove_clion_opencode_config(config):
    ...
    if servers.pop(config.get("JETBRAINS_AGENT_NAME", "OpenCode RunPod"), None) is None:
        return
...
def down():
    ...
    with open(manager.state_dir / "runtime.lock", "w") as lock:
        fcntl.flock(lock, fcntl.LOCK_EX)
        try:
            result = _systemctl("stop", "llm-coding-tunnel.service", check=False)
            if result.returncode:
                errors.append(f"could not stop the SSH tunnel ({_command_error(result)})")
        except OSError as exc:
        ...
        if not pod_name or not api_key:
            errors.append("could not stop the RunPod pod (set RUNPOD_POD_NAME and RUNPOD_API_KEY, then run llm-down again)")
        else:
            try:
        ...
        for generated_file in (manager.state_dir / "runtime.env", manager.state_dir / "opencode.json"):
            try:
...
def tunnel():
    manager = ConfigManager(); values = dict(line.strip().split("=", 1) for line in (manager.state_dir / "runtime.env").read_text().splitlines() if "=" in line)
    config = manager.load_config(); known_hosts = manager.state_dir / "known_hosts"
    os.execvp("ssh", ["ssh", "-N", "-T", "-i", values["SSH_KEY"], "-p", values["SSH_PORT"], "-o", "BatchMode=yes", "-o", "ExitOnForwardFailure=yes", "-o", "ServerAliveInterval=30", "-o", "ServerAliveCountMax=3", "-o", "StrictHostKeyChecking=accept-new", "-o", f"UserKnownHostsFile={known_hosts}", "-L", f"127.0.0.1:{config.get('LOCAL_TUNNEL_PORT', '18001')}:127.0.0.1:{config.get('REMOTE_VLLM_PORT', '8000')}", f"root@{values['SSH_HOST']}"])
