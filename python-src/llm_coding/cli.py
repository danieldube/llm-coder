...
from .core import ConfigManager, RunPodClient, fatal, check_dependencies, create_opencode_config
from .runtime import down as runtime_down, ensure_socket, _startup_timeout_seconds
...
def create_pod_config(config: Dict[str, str]) -> Dict:
    ...
    pod_config = {
        ...
        "volumeMountPath": config.get("RUNPOD_VOLUME_MOUNT_PATH", "/workspace"),
...
def ensure_vllm_on_remote(host: str, port: int, ssh_key: str, config: Dict[str, str]) -> bool:
    ...
    try:
        ...
        ensure_vllm_script = Path(__file__).parent.parent / "remote" / "ensure-vllm.sh"
        if not ensure_vllm_script.exists():
            # Fallback to embedded content or just copy what we need
            logger.warning("ensure-vllm.sh not found, using minimal implementation")
            return True
        ...
        with open(ensure_vllm_script, 'r') as f:
...
def find_pod_by_name(runpod_client: RunPodClient, pod_name: str) -> Optional[Dict]:
    """Find a pod by name"""
    try:
...
def runpod_api(runpod_client: RunPodClient, method: str, path: str, body: Optional[str] = None) -> str:
    """Make a RunPod API call"""
    return runpod_client.api_call(method, path, body)


def print_runtime_ready_summary(config: Dict[str, str], state_dir: Path, proxy_models: Dict) -> None:
    ...
    # reachable.
    if not any(item.get("id") == model for item in proxy_models.get("data", [])):
        fatal(f"Local proxy does not serve expected model {model}")
    try:
        tunnel_models = requests.get(
            f"http://127.0.0.1:{tunnel_port}/v1/models", timeout=5
        ).json().get("data", [])
    except (requests.RequestException, ValueError) as exc:
        fatal(f"Proxy responded, but the direct SSH tunnel check failed: {exc}")
    if not any(item.get("id") == model for item in tunnel_models):
        fatal(f"Proxy responded, but the SSH tunnel does not serve expected model {model}")

    unit_states = {
        service: get_systemd_service_info(service)[0]
        for service in ("llm-coding.socket", "llm-coding-proxy.service", "llm-coding-tunnel.service")
    }
    inactive = [service for service, state in unit_states.items() if state != "active"]
    if inactive:
        fatal("Endpoint responded, but required systemd unit(s) are not active: " + ", ".join(inactive))

    pod_summary = "API status unavailable"
    try:
        pod = RunPodClient(config["RUNPOD_API_KEY"]).find_pod_by_name(config["RUNPOD_POD_NAME"])
        if pod:
        ...
        logger.warning("Could not confirm RunPod pod status after activation: %s", exc)
    ...
    try:
        ...
        if config.get("JETBRAINS_AGENT_NAME", "OpenCode RunPod") in acp.get("agent_servers", {}):
    ...
    print(f"  CLion ACP:      {config.get('JETBRAINS_AGENT_NAME', 'OpenCode RunPod')} ({acp_state})")
...
def _shutdown_summary_items():
    ...
    for unit, label in (
        ...
        try:
            result = run_command(
                ["systemctl", "--user", "is-active", unit], capture_output=True, check=False
            )
    ...
    except (OSError, ValueError, json.JSONDecodeError, requests.RequestException, RuntimeError):
...
def llm_up():
    ...
    if not config_manager.config_file.is_file() or not config_manager.secrets_file.is_file():
    ...
    try:
        ...
        if initial_state == "active":
            ...
            try:
                ...
                print("Recovering an active proxy with an unavailable SSH tunnel...", file=sys.stderr)
                result = run_command(
                ...
                if result.returncode != 0:
                    details = result.stderr.strip() if result.stderr else "unknown systemd error"
                    if print_activation_failure(config_manager.state_dir):
                        sys.exit(1)
                    fatal(f"Could not restart the stale llm-coding proxy: {details}")
        ...
        request = subprocess.Popen(
            [
                ...
                "--max-time", str(timeout_seconds), endpoint,
            ],
        ...
        while request.poll() is None:
            activation_status = read_activation_status(config_manager.state_dir)
            if activation_status and activation_status != last_activation_status:
                print(f"  {activation_status}...", file=sys.stderr)
                last_activation_status = activation_status

            # Get current service state
            active_state, current_invocation = get_systemd_service_info("llm-coding-proxy.service")
            
            # Check if activation has been observed
            if not activation_observed and \
               (active_state == "activating" or active_state == "active") and \
               current_invocation and \
            ...
            if activation_observed and (active_state == "failed" or 
                                       (current_invocation and current_invocation != proxy_invocation)):
                request.terminate()
                ...
                if not print_activation_failure(config_manager.state_dir):
                    print("LLM runtime activation failed. See: journalctl --user --unit llm-coding-proxy.service --lines 30 --no-pager", file=sys.stderr)
                sys.exit(1)
        ...
        try:
            ...
            print(response, end='')
            return
        print_runtime_ready_summary(config, config_manager.state_dir, proxy_models)
...
def llm_down():
    ...
    for unit, label in (
        ...
        try:
            ...
            if result.returncode:
                ...
                errors.append(f"could not stop {label} ({detail.splitlines()[-1] if detail else 'unknown systemd error'})")
    ...
    if errors:
        fatal("Shutdown incomplete. " + "; ".join(errors) + ". Fix the listed issue and run llm-down again.")
    for item in stopped:
...
def llm_status():
    ...
    for service, label in services:
        try:
            result = run_command(["systemctl", "--user", "is-active", service], capture_output=True)
    ...
    try:
        ...
        if pod:
            ...
            gpu = pod.get("gpu", {}).get("displayName") or pod.get("gpu", {}).get("id", "unknown")
...
def llm_doctor(activate: bool):
    ...
    print(f"vLLM CUDA variant:   cu{config.get('VLLM_CUDA_VERSION', 'unknown')}")
    ...
    # Check SSH keys
    ssh_key_path = Path(os.path.expandvars(os.path.expanduser(config.get("RUNPOD_SSH_KEY", ""))))
    if not ssh_key_path.exists() or not (ssh_key_path.with_suffix(".pub")).exists():
    ...
    if expected_version:
        installed_version = run_command([str(opencode_bin), "--version"]).stdout.strip()
        if installed_version != expected_version:
            fatal(f"OpenCode version {installed_version}; expected {expected_version}")
    print("OpenCode installed")
    
    # Check systemd socket activation
    try:
        result = run_command(["systemctl", "--user", "is-enabled", "llm-coding.socket"], capture_output=True, check=False)
        if result.returncode == 0:
            result = run_command(["systemctl", "--user", "is-active", "llm-coding.socket"], capture_output=True, check=False)
            if result.returncode == 0:
    ...
    try:
        acp = json.loads(acp_file.read_text())
        if config.get("JETBRAINS_AGENT_NAME") not in acp.get("agent_servers", {}):
    ...
    if not activate:
        print("Use 'llm-doctor --activate' for an end-to-end RunPod/vLLM/tool-calling test.")
        return
    ...
    try:
        ...
        endpoint = f"http://127.0.0.1:{config.get('LOCAL_PROXY_PORT', '18000')}/v1"
        models = requests.get(endpoint + "/models", timeout=30).json().get("data", [])
        model = config.get("SERVED_MODEL_NAME", "unknown")
        if not any(item.get("id") == model for item in models):
            fatal("Expected model is not served")
        print("Expected model " + model)
        completion = requests.post(endpoint + "/chat/completions", timeout=60, json={
            "model": model, "max_tokens": 32,
            "messages": [{"role": "user", "content": "Reply with exactly: OK"}],
        }).json()
        if not completion.get("choices", [{}])[0].get("message", {}).get("content"):
            fatal("Chat completion failed")
        print("Chat completion")
        tool_response = requests.post(endpoint + "/chat/completions", timeout=60, json={
        ...
        if not tool_response.get("choices", [{}])[0].get("message", {}).get("tool_calls"):
...
def opencode_runpod(ctx):
    ...
    endpoint = f"http://127.0.0.1:{config.get('LOCAL_PROXY_PORT', '18000')}/v1/models"
    subprocess.Popen(
        ["curl", "--fail", "--silent", "--max-time", str(
            int(config.get("RUNPOD_START_TIMEOUT_SECONDS", 1200))
            + int(config.get("VLLM_START_TIMEOUT_SECONDS", 1800)) + 120), endpoint],
