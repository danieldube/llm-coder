...
class ConfigManager:
    ...
    def __init__(self):
        self.config_dir = Path(os.environ.get('XDG_CONFIG_HOME', '~/.config')).expanduser() / 'llm-coding'
        self.state_dir = Path(os.environ.get('XDG_STATE_HOME', '~/.local/state')).expanduser() / 'llm-coding'
        self.install_dir = Path(os.environ.get('XDG_DATA_HOME', '~/.local/share')).expanduser() / 'llm-coding'
...
def create_opencode_config(config: Dict[str, str], state_dir: Path) -> Path:
    ...
    modified_config["model"] = f"runpod/{config.get('SERVED_MODEL_NAME', 'PLACEHOLDER')}"
    
    # Update provider options
    modified_config["provider"]["runpod"]["options"]["baseURL"] = f"http://127.0.0.1:{config.get('LOCAL_PROXY_PORT', 18000)}/v1"
